...
function App() {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await invoke("save_config", { serverUrl: String(form.get("serverUrl") ?? ""), token: String(form.get("token") ?? "") });
      setSaved(true);
