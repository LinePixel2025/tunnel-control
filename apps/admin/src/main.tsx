...
import { Activity, Cable, CirclePlus, Computer, LogIn, Power, RefreshCw, ShieldCheck } from "lucide-react";
...
function App() {
  ...
  const [activeView, setActiveView] = useState<"overview" | "tunnels" | "devices">("overview");
  ...
  const refresh = async () => {
    try {
      const [nextSummary, nextDevices, nextTunnels] = await Promise.all([request<Summary>("/summary"), request<Device[]>("/devices"), request<Tunnel[]>("/tunnels")]);
      setSummary(nextSummary); setDevices(nextDevices); setTunnels(nextTunnels); setError("");
  ...
  return <div className="app-shell"><aside><div className="brand"><span className="brand-mark"><Cable size={20}/></span><span>Tunnel<br/><b>Control</b></span></div><nav><button type="button" className={activeView === "overview" ? "nav-active" : ""} aria-current={activeView === "overview" ? "page" : undefined} onClick={() => setActiveView("overview")}><Activity size={17}/>运营概览</button><button type="button" className={activeView === "tunnels" ? "nav-active" : ""} aria-current={activeView === "tunnels" ? "page" : undefined} onClick={() => setActiveView("tunnels")}><Cable size={17}/>公网隧道</button><button type="button" className={activeView === "devices" ? "nav-active" : ""} aria-current={activeView === "devices" ? "page" : undefined} onClick={() => setActiveView("devices")}><Computer size={17}/>Windows 设备</button></nav><div className="secure"><ShieldCheck size={16}/><span>管理控制面<br/><b>管理员会话已验证</b></span></div></aside><main><header><div><p className="eyebrow">默认工作区</p><h1>{activeView === "overview" ? "隧道运营" : activeView === "tunnels" ? "公网隧道" : "Windows 设备"}</h1></div><div className="header-actions"><span className="online-dot"/>服务运行中<button className="icon-button" title="刷新数据" onClick={refresh}><RefreshCw size={16}/></button><button className="text-button" onClick={() => { localStorage.removeItem("tunnel-admin-token"); setToken(""); }}>退出</button></div></header>{error && <div className="notice"><b>连接提示</b>{error}</div>}<section className="metrics"><Metric label="在线设备" value={`${summary?.online_devices ?? 0} / ${summary?.devices ?? 0}`} icon={<Computer size={21}/>}/><Metric label="启用隧道" value={`${tunnels.filter(t => t.enabled).length}`} icon={<Cable size={21}/>}/><Metric label="活动连接" value={`${summary?.active_connections ?? 0}`} icon={<Activity size={21}/>}/></section>{activeView !== "devices" && <section className="panel"><div className="panel-heading"><div><h2>公网隧道</h2><p>由管理员分配端口，并转发到指定 Windows 设备的本地服务。</p></div><button className="primary" onClick={() => setShowForm(true)}><CirclePlus size={16}/>新建隧道</button></div>{tunnels.length ? <div className="table"><div className="row label"><span>名称</span><span>公网入口</span><span>本地目标</span><span>设备</span><span>状态</span><span/></div>{tunnels.map(tunnel => <div className="row" key={tunnel.id}><b>{tunnel.name}<small>{tunnel.kind.toUpperCase()}</small></b><code>:{tunnel.public_port}</code><code>{tunnel.local_host}:{tunnel.local_port}</code><span>{devices.find(device => device.id === tunnel.device_id)?.name ?? "未知设备"}</span><span className={`status ${tunnel.enabled ? "ready" : "off"}`}>{tunnel.enabled ? tunnel.status : "已停用"}</span><button className="icon-button" title={tunnel.enabled ? "停用隧道" : "启用隧道"} onClick={() => toggle(tunnel.id)}><Power size={16}/></button></div>)}</div> : <Empty onCreate={() => setShowForm(true)}/>}</section>}{activeView !== "tunnels" && <section className="panel devices-panel"><div className="panel-heading"><div><h2>设备状态</h2><p>设备需使用服务端签发的访问令牌连接。</p></div></div>{devices.length ? devices.map(device => <div className="device" key={device.id}><span className={`device-dot ${device.status}`}/><div><b>{device.name}</b><p>{device.id.slice(0, 8)} · {device.latency_ms} ms</p></div><span className={`status ${device.status === "online" ? "ready" : "off"}`}>{device.status === "online" ? "在线" : "离线"}</span></div>) : <div className="device-empty">尚无设备。请先在 Windows 客户端中填写控制通道地址与设备令牌。</div>}</section>}</main>{showForm && <TunnelForm devices={devices} onClose={() => setShowForm(false)} onSubmit={createTunnel}/>}</div>;
}
