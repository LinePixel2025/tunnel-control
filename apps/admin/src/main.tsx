...
import { Activity, Cable, Check, CirclePlus, Computer, Copy, KeyRound, LogIn, Power, RefreshCw, ShieldCheck } from "lucide-react";
...
type Tunnel = { id: string; name: string; kind: "tcp" | "http"; public_port: number; local_host: string; local_port: number; enabled: boolean; device_id: string; status: string; connections: number };
...
function TunnelsPanel({ tunnels, devices, onToggle, onCreate }: { tunnels: Tunnel[]; devices: Device[]; onToggle: (id: string) => void; onCreate: () => void }) {
  return <section className="panel"><div className="panel-heading"><div><h2>公网隧道</h2><p>由管理员分配端口，并转发到指定 Windows 设备的本地服务。</p></div><button className="primary" onClick={onCreate}><CirclePlus size={16}/>新建隧道</button></div>{tunnels.length ? <div className="table"><div className="row label"><span>名称</span><span>公网入口</span><span>本地目标</span><span>设备</span><span>状态</span><span/></div>{tunnels.map(tunnel => <div className="row" key={tunnel.id}><b>{tunnel.name}<small>{tunnel.kind.toUpperCase()}</small></b><code>:{tunnel.public_port}</code><code>{tunnel.local_host}:{tunnel.local_port}</code><span>{devices.find(device => device.id === tunnel.device_id)?.name ?? "未知设备"}</span><span className={`status ${tunnel.enabled ? "ready" : "off"}`}>{tunnel.enabled ? tunnel.status : "已停用"}</span><button className="icon-button" title={tunnel.enabled ? "停用隧道" : "启用隧道"} onClick={() => onToggle(tunnel.id)}><Power size={16}/></button></div>)}</div> : <Empty onCreate={onCreate}/>}</section>;
}
...
function KeysPanel({ keys, onCreate, onRevoke }: { keys: AccessKey[]; onCreate: () => void; onRevoke: (key: AccessKey) => void }) {
  return <section className="panel"><div className="panel-heading"><div><h2>接入密钥</h2><p>密钥用于客户端连接控制通道；未绑定设备的密钥在首次连接时自动注册设备。</p></div><button className="primary" onClick={onCreate}><CirclePlus size={16}/>新建密钥</button></div>{keys.length ? <div className="table keys-table"><div className="row label"><span>名称</span><span>设备</span><span>状态</span><span>创建时间</span><span>过期时间</span><span>最后使用</span><span/></div>{keys.map(key => <div className="row" key={key.id}><b>{key.label}</b><span>{key.device_name ?? "未绑定"}</span><span className={`status ${key.status === "active" ? "ready" : "off"}`}>{keyStatusLabel[key.status]}</span><span>{formatDate(key.created_at)}</span><span>{key.expires_at ? formatDate(key.expires_at) : "不过期"}</span><span>{formatDate(key.last_used_at)}</span><button className="icon-button" title="撤销密钥" disabled={key.status !== "active"} onClick={() => onRevoke(key)}><Power size={16}/></button></div>)}</div> : <div className="device-empty">尚未创建接入密钥。创建后把令牌填入 Windows 客户端即可连接。</div>}</section>;
}
function KeyForm({ devices, onClose, onSubmit }: { devices: Device[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="modal-backdrop"><form className="modal" onSubmit={onSubmit}><div className="modal-head"><div><p className="eyebrow">管理员操作</p><h2>新建接入密钥</h2></div><button type="button" className="text-button" onClick={onClose}>关闭</button></div><label>名称<input required name="label" maxLength={100} placeholder="例如：研发笔记本"/></label><label>绑定设备<select name="device_id" defaultValue=""><option value="">首次连接时自动注册新设备</option>{devices.map(device => <option key={device.id} value={device.id}>{device.name}</option>)}</select></label><label>有效期<select name="expires_in_days" defaultValue=""><option value="">不过期</option><option value="7">7 天</option><option value="30">30 天</option><option value="90">90 天</option><option value="365">365 天</option></select></label><div className="modal-actions"><button type="button" className="text-button" onClick={onClose}>取消</button><button className="primary" type="submit">创建密钥</button></div></form></div>;
}
...
function TunnelForm({ devices, onClose, onSubmit }: { devices: Device[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <div className="modal-backdrop"><form className="modal" onSubmit={onSubmit}><div className="modal-head"><div><p className="eyebrow">管理员操作</p><h2>新建隧道</h2></div><button type="button" className="text-button" onClick={onClose}>关闭</button></div><label>名称<input required name="name" placeholder="例如：研发远程桌面"/></label><div className="two"><label>类型<select name="kind"><option value="tcp">TCP</option><option value="http">HTTP</option></select></label><label>公网端口<input name="public_port" required type="number" min="1" max="65535" placeholder="10001"/></label></div><label>目标设备<select required name="device_id" defaultValue=""><option value="">选择在线设备</option>{devices.filter(device => device.status === "online").map(device => <option key={device.id} value={device.id}>{device.name}</option>)}</select></label><div className="two"><label>本地地址<input name="local_host" required defaultValue="127.0.0.1"/></label><label>本地端口<input name="local_port" required type="number" min="1" max="65535" placeholder="3389"/></label></div><label>最大并发<input name="max_connections" type="number" min="1" max="1000" defaultValue="100"/></label><div className="modal-actions"><button type="button" className="text-button" onClick={onClose}>取消</button><button className="primary" type="submit">创建隧道</button></div></form></div>; }
